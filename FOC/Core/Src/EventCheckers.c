...
bool Check4Keystroke(void)
{
  ...
  if ( IsNewKeyReady() ) // new key waiting?
  {
	...
	ThisEvent.EventType = ES_DEFAULT;
	return false;
  }
}

bool Check4CANRx(void)
{
   ES_Event ThisEvent;
   switch(CANFlag)
   {
   	   case 1: // calibrate HES
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'k';
   	       PostFSM( ThisEvent );

   		   break;
   	   case 2: // turn off PI controller
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'o';
   	       PostFSM( ThisEvent );

   		   break;
   	   case 3: // run voltage FOC
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'f';
   	       PostFSM( ThisEvent );

   		   break;
   	   case 4: // turn on PI controller
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'm';
   	       PostFSM( ThisEvent );

   		   break;
   	   case 5: // turn on speed PI controller
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'w';
   	       PostFSM( ThisEvent );

   		   break;
   	   case 7: // print variables
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'c';
   	       PostFSM( ThisEvent );

   		   break;
   	   case 8: // turn on current PI controller
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'x';
   	       PostFSM( ThisEvent );

   		   break;
   	   case 9: // turn on torque PI controller
   		   ThisEvent.EventType = ES_NEW_KEY;
   	       ThisEvent.EventParam = 'z';
   	       PostFSM( ThisEvent );
   ...
   if(CANFlag > 0)
   {
	   CANFlag = 0; // reset the flag to zero
	   return true;
   }
   else
   {
...
//bool Check4Timeout(void)
//{
//  ES_Event ThisEvent;
//  if ( Timeout() ) // new key waiting?
//  {
