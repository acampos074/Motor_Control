...
****************************************************************************/
//bool _HW_Process_Pending_Ints( void ) // moved function to stm32f4xx_it.c
//{
//   while (TickCount > 0)
//   {
...
{
  uint32_t r0;
  //__asm("MRS %0, primask" : "=r" (r0) );
  //__asm("MRS %0, primask" : "=r" (r0) :: "memory" );
  r0 = __get_PRIMASK();
...
void CPUsetPRIMASK(uint32_t newPRIMASK)
{
  //__asm("MSR primask, %0" : : "r" (newPRIMASK) : );
  //__asm("MSR primask, %0" : : "r" (newPRIMASK) : "memory");
	__set_PRIMASK(newPRIMASK);
  // Store newPRIMASK in PRIMASK
}

uint32_t CPUgetFAULTMASK_cpsid(void)
{
  uint32_t r0;
  //__asm("MRS %0, faultmask" : "=r" (r0) );

  r0 = __get_FAULTMASK();
  __asm("cpsid f" : : : "memory");
  //__disable_fiq();
  // Store FAULTMASK in r0
...
void CPUsetFAULTMASK(uint32_t newFAULTMASK)
{
  //__asm("MSR faultmask, %0" : : "r" (newFAULTMASK) : "memory");
	__set_FAULTMASK(newFAULTMASK);
	// Store newFAULTMASK in FAULTMASK
}
#endif
